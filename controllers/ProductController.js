...
async function getProductByNumber(number) {
  const productNumber = number.trim();
  var product = await models.sequelize.query(
    "select p.id as productId, p.*, p.deleteFl as status, pt.type as productType from products p " +
      " inner join productTypes pt on p.productTypeFk = pt.id " +
      " where p.productNumber = :number ",
    {
      replacements: { number: productNumber },
...
exports.searchProductsResults = async function (req, res) {
  ...
  var query =
    "select *, pt.type as productType, p.deleteFl as status, t.* as template, pr.price1 from products p " +
    " inner join productTypes pt on p.productTypeFk = pt.id " +
...
exports.getProductItemScreen = async function (req, res) {
  ...
  if (productItemNumber == undefined) {
    ...
    var existingProductItems = await getProductItemsWithProductForAccount(
      productId,
      account.id
    );
    var kid = null;

    var kids = await kidController.getKidsFromAccountIdAndProductId(
      account.id,
      productId
    );
    ...
    if (kidCode != undefined) {
      ...
      existingProductItems = await getProductItemsWithProductForAccount(
        productId,
        account.id,
        kid.id
      );
    ...
    if (productItem.productVariantFk != productVariantId)
      throw (
        "ProductVariantId doesn't match productItem with number " +
        productItemNumber
      );
  ...
  var kids = await kidController.getKidsForProductAndAccount(
    productId,
    account.id
  );
  var kid = null;
  ...
  kids.forEach((kid) => {
    if (kid.id != productItem.kidFk) kidsList.push(kid);
  });

  if (kids != null) kid = kidsList[0];

  var productVariants = await getProductVariantsForProductItemGroupId(
  ...
  res.render("productItem2", {
    ...
    basketItemsDetails: basketItemsDetails,
    product: product,
...
async function createProductItems(product, kidCode, account) {
  ...
  var isAccountLinkedToASchoolInScheme =
    await kidController.isAccountLinkedToASchoolInScheme(account.id);
  ...
  if (product.kidFl) {
    ...
    if (kidCode != undefined) {
      ...
      // generateCard
      // create productit
      productItemGroup = await productUtility.createProductItemGroup();
...
exports.getProductItemsWithProductForAccount = async function (
  productId,
  accountId,
  kidId
) {
  return await getProductItemsWithProductForAccount(
    productId,
    accountId,
    kidId
  );
...
async function getProductItemsWithProductForAccount(
  ...
  return await models.sequelize.query(query, {
    replacements: { kidId: kidId, productId: productId, accountId: accountId },
    type: models.sequelize.QueryTypes.SELECT,
  });
}
...
async function getProductItemsWithProductForKid(productId, kidId) {
  return await models.sequelize.query(
    "select pi.*, pv.orderNo, pv.price, pvt.type as productVariantType from productItems pi " +
      ...
      " and pv.deleteFl = false ",
    {
      replacements: { productId: productId, kidId: kidId },
      type: models.sequelize.QueryTypes.SELECT,
    }
  );
}

exports.getProductItemsWithProductForAccountAndNotWithKid = async function (
  productId,
  accountId
) {
  return await getProductItemsWithProductForAccountAndNotWithKid(
    productId,
    accountId
  );
};
...
async function getProductItemsWithProductForAccountAndNotWithKid(
  ...
  return await models.sequelize.query(
    "select pi.*, pv.orderNo, pv.price, pvt.type as productVariantType from productItems pi " +
      ...
      " and pv.deleteFl = false ",
    {
      replacements: { productId: productId, accountId: accountId },
      type: models.sequelize.QueryTypes.SELECT,
    }
  );
}
...
async function generateUpdateProductItem(kid, productId, accountId) {
  ...
  if (kid == null) {
    ...
    kidSchoolClassDetails = await kidController.getKidClassAndSchoolFromKidId(
      kid.id
    );
  ...
  for (var i = 0; i < productItems.length; i++) {
    ...
    var productVariantItem = await getProductVariantForProductItemId(
      productItem.id
    );
...
async function generateProductItemNoKid(
  ...
  for (var i = 0; i < productVariants.length; i++) {
    ...
    for (var j = 0; j < productVariantItem.pictureCount; j++) {
      var index = j + 1;
      data["picture" + index] =
        productVariantItem["defaultPicture" + index + "Path"];
    }

    data["accountId"] = accountId;
    var productItem = !dummy
...
exports.generateProductItemForKid = async function (
  kid,
  productId,
  dummy,
  isAccountLinkedToASchoolInScheme
) {
  return await generateProductItemForKid(
    kid,
    productId,
    dummy,
    isAccountLinkedToASchoolInScheme
...
async function generateProductItemForKid(
  ...
  var kidSchoolClassDetails = await kidController.getKidClassAndSchoolFromKidId(
    kid.id
  );
...
async function getProductVariantsForProductId(productId) {
  return await models.sequelize.query(
    "select pv.id as productVariantId, pvt.type as productVariantType,  pv.name as productVariantName, pv.*, t.id as templateId, t.*, t.name as templateName from productVariants pv " +
      ...
      " inner join productVariantTypes pvt on pv.productVariantTypeFk = pvt.id " +
      " where pv.productFk = :productId order by pv.orderNo asc",
    {
...
async function getProductVariantForProductItemId(productItemId) {
  var result = await models.sequelize.query(
    ...
    {
      replacements: { productItemId: productItemId },
      type: models.sequelize.QueryTypes.SELECT,
    }
  );
...
async function createProductItemForKidPdf(
  ...
  return await createProductItemObject(
    productVariantItem,
    data,
    kid,
    s3Path,
...
async function createProductItemObjectNoKid(
  ...
  var productItemNumber = await productItemUtility.getNewProductItemNumber();
  var object = {
  ...
  try {
    productItem = await productItemUtility.createProductItem(object);
  } catch (err) {
    console.log(err);
...
async function getProductVariantById(productVariantId) {
  var result = await models.sequelize.query(
    "select pv.*, pvt.type as productVariantType from productVariants pv " +
      " inner join productVariantTypes pvt on pv.productVariantTypeFk = pvt.id " +
      " where pv.id = :productVariantId " +
      " and pv.deleteFl = false ",
    {
      replacements: { productVariantId: productVariantId },
      type: models.sequelize.QueryTypes.SELECT,
    }
  );
  return result.length == 0 ? null : result[0];
...
async function getProductItemByProductVariant(
  productVariantId,
  accountId,
  kidCode
) {
  var query =
    "select pi.*, pvt.type as productVariantType, pv.price from productItems pi " +
    " inner join productVariants pv on pi.productVariantFk = pv.id " +
    " inner join productVariantTypes pvt on pv.productVariantTypeFk = pvt.id " +
    " where pv.id = :productVariantId " +
    " and pi.accountFk = :accountId " +
...
exports.uploadAndGenerate = async function (req, res) {
  ...
  var job = await queueController.addUploadAndGenerateJob(
    productItemId,
    req.body.pictureNumber,
    req.body.productId,
    req.files
  );
  res.json({
...
exports.updateAndGenerate = async function (req, res) {
  var productItemId = req.body.productItemId;
  var job = await queueController.addUpdateAndGenerateJob(
    productItemId,
    req.body.productId,
    req.body.name,
    req.body.age,
    req.body.month,
    req.body.displaySchool == "true",
    req.body.displayClass == "true",
    req.body.displayAge == "true"
  );
  ...
  res.json({
    ...
    productNumber: req.body.productNumber,
    productVariantId: req.body.productVariantId,
    productItemNumber: productItem.productItemNumber,
    totalSteps: 4,
  });
};
...
exports.doesProductItemStillHaveDefaultPictures = async function (productItem) {
  ...
  var productVariant = await getProductVariantById(
    productItem.productVariantFk
  );
...
exports.getPriceForProductItemId = async function (id) {
  var price = await models.sequelize.query(
...
exports.getProductVariantsForProductItemGroupId = async function (
  productItemGroupId
) {
...
async function getProductVariantsForProductItemGroupId(productItemGroupId) {
  return await models.sequelize.query(
    "select distinct pv.id as productVariantId, pi.productItemNumber, pvt.type as productVariantType,  pv.name as productVariantName, pv.* from productItems pi " +
      " inner join productItemGroups pis on pi.productItemGroupFk = pis.id " +
      " inner join productVariants pv on pi.productVariantFk = pv.id " +
      " inner join productVariantTypes pvt on pv.productVariantTypeFk = pvt.id " +
      " where pis.id = :productItemGroupId ",
    {
      replacements: { productItemGroupId: productItemGroupId },
      type: models.sequelize.QueryTypes.SELECT,
    }
  );
}
...
async function getProductItemsForKidNumber(kidNumber) {
  var query =
    " select pi.*, pv.orderNo, p.productNumber, pv.id as productVariantId, p.name as productName, p.displayImagePath, pt.type as productType, pv.name as productVariantName, pvt.type as productVariantType from productItems pi " +
    " inner join productVariants pv on pi.productVariantFk = pv.id " +
    " inner join productVariantTypes pvt on pv.productVariantTypeFk = pvt.id " +
  ...
  return await models.sequelize.query(query, {
    replacements: { kidNumber: kidNumber },
    type: models.sequelize.QueryTypes.SELECT,
  });
}
