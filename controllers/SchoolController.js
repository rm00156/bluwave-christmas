...
exports.getSearchSchoolsPage = async function (req, res) {
  ...
  var schoolsRequiringGiveBackAction =
    await schoolUtility.getSchoolsRequiringGiveBackAction();

  res.render("newAdminSchools", {
    user: req.user,
    statusTypes: statusTypes,
    ordersNotShipped: ordersNotShipped,
...
exports.getSchoolPage = async function (req, res) {
  ...
  var giveBackTotal =
    giveBackDetails == null ? 0.0 : giveBackDetails.giveBackTotal;
  var nextSteps = await schoolUtility.getNextStepsForStatusType(
    school.nextTypeFk,
    school.typeId
  );

  var classes = await classUtility.getClassDetailsFromSchoolNumber(
    schoolNumber
  );
  ...
  var orderDetails = await schoolUtility.getOrderDetailsForAllKidsFromSchoolId(
    school.id
  );
  var backgroundSetting = await adminUtility.getBackgroundSetting(req.user.id);
  var ordersNotShipped = await orderUtility.getOrdersNotShipped();
  var statusTypeDetails = await schoolUtility.getStatusTypeDetailsForSchoolId(
    school.id
  );
  var schoolsRequiringGiveBackAction =
    await schoolUtility.getSchoolsRequiringGiveBackAction();
  ...
  res.render("newSchoolDetail", {
    ...
    daysLeft: daysLeft,
    daysLeftSign: daysLeftSign,
    nextSteps: nextSteps,
    giveBackTotal: giveBackTotal,
...
exports.searchSchoolsResults = async function (req, res) {
  ...
  var searchResult =
    await schoolUtility.searchForSchoolByNameAddressPostCodeStatusAndEmail(
      nameSearch,
      addressSearch,
      postCodeSearch,
      status,
...
exports.updateSchoolDetailsForSchoolId = async function (
  schoolId,
  schoolName,
  address,
  postCode,
  ...
  try {
    ...
    console.log(err);
    if (err) return await t.rollback();
  }
...
exports.changeSchoolStep = async function (req, res) {
  ...
  var statusDetail = { nextTypeFk: null };
  if (nextTypeFk == "") {
    const statusDetails =
      await schoolUtility.getCurrentSchoolsStatusDetailsBySchoolId(schoolId);

    if (statusDetails !== null) {
      statusDetail = statusDetails[0];
    }
  } else {
...
exports.getGiveBacksScreen = async function (req, res) {
  ...
  var giveBackAmountDetailsForEachSchool =
    await schoolUtility.getGiveBackAmountDetailsForEachSchool();
  var schoolsRequiringGiveBackAction =
    await schoolUtility.getSchoolsRequiringGiveBackAction();

  res.render("giveBackAmounts", {
    user: req.user,
    giveBackAmountDetailsForEachSchool: giveBackAmountDetailsForEachSchool,
    backgroundSetting: backgroundSetting,
    ordersNotShipped: ordersNotShipped,
    schoolsRequiringGiveBackAction: schoolsRequiringGiveBackAction,
  });
...
exports.continue = async function (req, res) {
  ...
  var deadline = await schoolUtility.getSchoolDeadlineByVerificationCode(
    verificationCode
  );
  ...
  if (deadline.continueFl == true) {
    ...
    errors.push(
      "We have already received a response and the status for " +
        school.name +
        " is already at " +
        (deadline.continueFl == true ? "Printing" : "Delayed")
    );
    //break
    return res.render("continue2", {
      user: req.user,
...
exports.setDeadLine = async function (req, res) {
  ...
  if (deadLine == null) {
    var verificationCode =
      await schoolUtility.createVerificationCodeForDeadline();
    // means new entry
...
exports.delay = async function (req, res) {
  ...
  const isSchoolStartedPrintingOrBeenDelayed =
    await schoolUtility.hasSchoolStartedPrintingOrBeenDelayed(school.id);
...
exports.linkKidByProductItemId = async function (req, res) {
  ...
  var classAndSchool = await classUtility.getClassAndSchoolByClassNumber(
    classCode
  );
...
exports.confirmAmount = async function (req, res) {
  ...
  var giveBackAmountBreakDownPerClass =
    await schoolUtility.getGiveBackAmountBreakDownPerClass(school.id);
  ...
  res.render("confirmAmount2", {
    ...
    classes: classes,
    totalGiveBackAmount: totalGiveBackAmount,
...
exports.getGiveBackDetailsScreen = async function (req, res) {
  ...
  var schoolRequiresAction = schoolsRequiringGiveBackAction.filter(
