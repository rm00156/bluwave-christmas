...
describe('school utility tests', () => {
    ...
    it('create new class', async () => {
        ...
        const school = schoolAccount.school;
    ...
    it('create new school by name', async () => {
        ...
        const school = await schoolUtility.createSchool(SCHOOL_NAME + 2, SCHOOL_NUMBER + 2, SCHOOL_ADDRESS,
            POST_CODE, SCHOOL_NUMBER, SCHOOL_EMAIL, null, 10, account.id);
    ...
    it('update school details by id', async () => {
        ...
        await schoolUtility.updateSchoolById(school.id, newName, newAddress, newPostCode, newNumber,
            newAdditionalInfo, newNumberOfKidsPerClass);
    ...
    it('return kids who are linked with any school', async () => {
        ...
        for(var i = 0; i < 5; i++) {
            kids.push(await kidUtility.createKid('John', 1, 0, schoolClass.id, account.id, 'abcd' + i));
        }
    ...
    it('return kids who are linked with school by school id', async () => {
        ...
        const kids = new Array();
        for(var j = 0; j < 5; j++) {
            kids.push(await kidUtility.createKid('John', 1, 0, schoolClass.id, account.id, 'kid_' + j));
        }
    ...
    it('get order details for all kids from a school by id', async () => {
        ...
        const kid = await kidUtility.createKid('John', 1, 0, schoolClass.id, account.id, `kid_order_details`);
        await kidUtility.createKid('John', 1, 0, schoolClass.id, account.id, `kid_order_details_2`);
        ...
        const product = await productUtility.createProduct('12345', 1, '4', '', '', false, false);
    ...
    it('get classes with orders', async () => {
        ...
        const kid = await kidUtility.createKid('John', 1, 0, schoolClass.id, account.id, `kid_order_details_`);
        await kidUtility.createKid('John', 1, 0, schoolClass.id, account.id, `kid_order_details_2`);
    ...
    it('get breakdown for standard pack per class by school', async () => {
        ...
        const classesData = givebackBreakdown.classesData;
    ...
    it('get breakdown for calendars per class by school', async () => {
        ...
        const productItem = await productItemTestHelper.generateProductItem(account, kid, productVariantType.id, TEMPLATES.CALENDAR_LANDSCAPE_BLUE, calendarProductType);
        const quantity = 8;
        const basketItem = await basketUtility.createBasketItem(account.id, '', productItem.id, '', '', quantity, '2', '', false, false, false);
        const purchaseBasket = await basketUtility.createPurchaseBasket(PURCHASE_BASKET_STATUS.COMPLETED, 2, 2, '', '', null);
    ...
    it('get the givebackamount and details for a school by id', async () => {
        ...
        const calendarProductItem = await productItemTestHelper.generateProductItem(account, kid, productVariantType.id, TEMPLATES.CALENDAR_LANDSCAPE_BLUE, calendarProductType);
        const photoPackProductItem = await productItemTestHelper.generateProductItem(account, kid, productVariantType.id, TEMPLATES.CHRISTMAS_SCHEME_CARD_PICTURE, photoPackProductType);
        const standardPackProductItem = await productItemTestHelper.generateProductItem(account, kid, productVariantType.id, TEMPLATES.CHRISTMAS_SCHEME_CARD, standardPackProductType);
