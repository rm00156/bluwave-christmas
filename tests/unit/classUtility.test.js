...
describe('class utility tests', () => {
  ...
  it('get class by id', async () => {
    ...
    const classDetails = await schoolTestHelper.createNewSchoolWithClass();
    const schoolClass = classDetails.class;
    const getClass = await classUtility.getClassById(schoolClass.id);

    expect(getClass.id).toBe(schoolClass.id);
  })
});
