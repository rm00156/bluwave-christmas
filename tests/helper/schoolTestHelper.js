...
async function createNewSchoolWithAccount() {
    ...
    const school = await schoolUtility.createSchool(schoolDetails.name, schoolDetails.schoolNumber, schoolDetails.address,
        schoolDetails.postCode, schoolDetails.telephone, organiserAccount.email, schoolDetails.additionalInfo, schoolDetails.numberOfKidsPerClass,
        organiserAccount.id);
...
async function createNewSchoolWithKid() {
    ...
    const kidCode = await kidUtility.generateNewKidCode();
    const kid = await kidUtility.createKid(KID_NAME, 1, 1, schoolClass.id, account.id, kidCode);

    return {...schoolClassDetails, ...{kid: kid}};
}
