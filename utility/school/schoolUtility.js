...
async function getCurrentSchoolsStatusDetailsBySchoolId(schoolId) {
  var query =
    "select s.id as schoolId, s.schoolNumber,s.name, st.id as statusId, stt.id as statusTypeId, stt.type as currentStep, stt.nextTypeFk from schools s " +
  ...
  const statusDetails = await models.sequelize.query(query, {
    replacements: { schoolId: schoolId },
    type: models.sequelize.QueryTypes.SELECT,
  });
  return statusDetails.length > 0 ? statusDetails : null;
}
...
async function getSchoolsRequiringGiveBackAction(schoolId) {
  ...
  var currentSchoolsStatusDetails =
    await getCurrentSchoolsStatusDetailsBySchoolId(schoolId);

  if (currentSchoolsStatusDetails === null) return [];
  ...
  return await models.sequelize
    .query(
        ...
        " and pb.status = :completed ",
      {
        replacements: {
          schoolId: schoolId,
          completed: PURCHASE_BASKET_STATUS.COMPLETED,
        },
        type: models.sequelize.QueryTypes.SELECT,
      }
    )
...
async function getSchoolFromBasketItemId(basketItemId) {
  var result = await models.sequelize.query(
    ...
    {
      replacements: { basketItemId: basketItemId },
      type: models.sequelize.QueryTypes.SELECT,
    }
  );
...
async function getGiveBackAmountBreakDownPerClass(schoolId) {
  ...
  for (var i = 0; i < classes.length; i++) {
    ...
    var giveBackAmountDetailsForClass =
      await classUtility.getGiveBackAmountDetailsForClassByClassId(
        schoolClass.id
      );
    ...
    totalGiveBackAmount =
      totalGiveBackAmount +
      parseFloat(giveBackAmountDetailsForClass.giveBackTotal);
...
async function getAllOrderItemsLinkedIndirectlyToSchoolId(schoolId) {
  var query =
    "select distinct b.id, pv.name, pt.type, b.quantity as quantity, b.cost from schools s " +
    " inner join classes c on c.schoolFk = s.id " +
...
async function getGiveBackAmount(schoolId) {
  ...
  const orderItemsDetailsLinkedIndirectlyToSchool =
    await getAllOrderItemsLinkedIndirectlyToSchoolId(schoolId);
...
async function getSchoolDashboardStatus() {
  ...
  var schoolsWithCurrentStatusComplete =
    await getSchoolsWithCurrentStatusComplete();
